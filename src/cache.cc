...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        // uint32_t cpu_evict = block[set][way].cpu;
        if (cache_type==IS_LLC)
        {
            if (block[set][way].cpu==fill_cpu && block[set][way].valid)
            {
                // cout<<"Incrementing self-evictions"<<endl;
                llc_self_evictions++;
            }
...
if((cache_type == IS_L1I || cache_type == IS_L1D) && reads_ready.size() == 0)
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
           ...
           //@vedant: Monitoring code
            if (cache_type==IS_LLC)
            {
                ...
                if (llc_accesses==1)
                {
                    //Initialise the dp_hash function and set_mapping structure
                    for (int k = 0; k < NUM_CPUS; k++)
                    {
                        dp_hash[k] = NUM_SET/NUM_CPUS;
                        dp_llc_sets_access_monitor[k] = 0;
                    }
                    for (int k = 0; k < NUM_CPUS; k++)
                    {
                        for (int j = 0; j < dp_hash[k]; j++)
                        {
                            dp_llc_mapped_sets[k][j] = ((RQ.entry[index].cpu*dp_hash[k]) + j);
                        }
                        
                    }
                }

                dp_llc_sets_access_monitor[RQ.entry[index].cpu]++;
                if (llc_accesses%100000==0)
                {
                    uint32_t victim_cpu = dp_get_min_accesses_cpu(RQ.entry[index].cpu);
                    if (RQ.entry[index].cpu!=victim_cpu)
                    {

                        //invalidate the sets as it remapped
                        // for (int set = cpu*dp_hash[cpu]; set <  cpu*dp_hash[cpu]+dp_hash[cpu]; set++)
                        // {
                        ...
                        //     }
                        // }
                        // for (int set = victim_cpu*dp_hash[victim_cpu]; set <  victim_cpu*dp_hash[victim_cpu]+dp_hash[victim_cpu]; set++)
                        // {
                        //     for (int way = 0; way < NUM_WAY; way++)
                        //     {
                        //         block[set][way].valid = false;
                        //     }
                        // }
                        for (int set = 0; set < dp_hash[RQ.entry[index].cpu]; set++)
                        {
                            for (int way = 0; way < NUM_WAY; way++)
                            {
                                block[dp_llc_mapped_sets[RQ.entry[index].cpu][set]][way].valid = false;
                            }
                        }
                        for (int set = 0; set < dp_hash[victim_cpu]; set++)
                        {
                            for (int way = 0; way < NUM_WAY; way++)
                            {
                                block[dp_llc_mapped_sets[victim_cpu][set]][way].valid = false;
                            }
                        }
                        
                        //Change the mapping and update the hash function
                        dp_llc_mapped_sets[RQ.entry[index].cpu][dp_hash[RQ.entry[index].cpu]] = dp_llc_mapped_sets[victim_cpu][dp_hash[victim_cpu]];
                        dp_hash[RQ.entry[index].cpu]++;
                        dp_hash[victim_cpu]--;

                        // cout<<"Change the Hash from "<<RQ.entry[index].cpu<<" --> "<<dp_hash[RQ.entry[index].cpu]<<" to "<<victim_cpu<<" --> "<<dp_hash[victim_cpu]<<endl;
                    }
       ...
       uint32_t CACHE::dp_get_min_accesses_cpu(uint32_t current_set){
            uint32_t min_set = current_set;
            uint64_t min = dp_llc_sets_access_monitor[current_set];

            // for (int i = 0; i < sizeof(dp_llc_sets_access_monitor); i++)
            for (int i = 0; i < NUM_CPUS; i++)
            {
                if (dp_hash[i]>NUM_CPUS && dp_llc_sets_access_monitor[i]<(0.8*min))
                {
                    min = dp_llc_sets_access_monitor[i];
                    min_set = i;
                }
        ...
        uint32_t CACHE::get_sp_hashed_set(uint32_t set){

            uint32_t hashed_set = (uint32_t)(((set%(NUM_SET/NUM_CPUS))*NUM_CPUS)+cpu);
            // cout << "cpu ki value " << cpu << endl;;
            assert(hashed_set%NUM_CPUS==cpu);
            return hashed_set;
            
        }

        uint32_t CACHE::get_dp_hashed_set(uint32_t set){

            // uint32_t hashed_set = (uint32_t)((set%(dp_hash[cpu]))+ ((dp_hash[cpu])*cpu));
            uint32_t hashed_set = (uint32_t)(set%(dp_hash[cpu]));
            // cout << "cpu ki value " << cpu << endl;;
            // assert(hashed_set%(NUM_SET/dp_hash[cpu])==cpu);
            return dp_llc_mapped_sets[cpu][hashed_set];
        ...
        uint32_t CACHE::get_wp_way(uint64_t address, uint32_t set)
        {
            uint32_t stride = cpu;

            for (uint32_t way=stride; way<NUM_WAY; way+=2) {
                if (block[set][way].valid && (block[set][way].tag == address)) 
                    return way;
            }
